...
export interface FireConfig {
  ...
  embers: boolean;
  fps: number; // 10–60 — simulation ticks per second (internal accumulator; independent of EngineOptions.fps)
...
export class FirePattern implements AsciiPattern {
  ...
  private flameSurges(): void {
    const regions = this.getFuelRegions();
    const surgeChance =
      this.config.mode === 'wall' ? 1 : this.config.mode === 'campfire' ? 0.5 : 0.3;
    const surgeCount = Math.random() < 0.5 * surgeChance ? 1 + Math.floor(Math.random() * 4) : 0;
