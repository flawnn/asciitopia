...
const createFlake = (
  ...
  const speedY =
    depth === 0 ? randomInRange(speedRange[0], mid) : randomInRange(mid, speedRange[1]);
...
export class SnowPattern implements AsciiPattern {
  ...
  init(cols: number, rows: number): void {
    ...
    this.flakes = Array.from({ length: count }, () => createFlake(cols, rows, this.config, true));
  }
  ...
  update(dt: number): void {
    ...
    for (const flake of this.flakes) {
      ...
      flake.x += Math.sin(flake.y * SWAY_FREQUENCY + flake.swayOffset) * flake.swayAmplitude * s;
      flake.x += wind * s;
    }
  }
